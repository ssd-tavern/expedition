/*
 * 远征-全屏前端.js —— 「富兰克林远征」角色卡的单文件全屏前端, 以酒馆助手(TavernHelper)脚本形式随卡分发,
 * 无构建步骤, 整份文件即部署产物。脚本运行在酒馆助手的 iframe 里, 操作的是 window.parent(酒馆页面)的 DOM;
 * 数据与规则来自配套的 MVU 变量系统与世界书。
 *
 ...
 *   数据表(技能图标/立绘库/地图与航海数据/角色名单/兜底常量/仪表工具)  约 1257-1452
 *   状态变量(立绘选取与角色选中态)                       约 1453-1481
 *   MVU读取与派生(readMVU/currentStat/previousStat)     约 1482-1574
 *   外壳骨架与切页(ensureShell/switchTab)               约 1575-1766
 *   动效工具(入场动画/外壳进出编排/数值变化反馈)          约 1767-2019
 *   设置项(主题/选项模式/立绘模式/字号/动效/SFW)         约 2020-2176
 *   角色与画廊(角色页/画廊/灯箱)                        约 2177-2410
 *   船员                                              约 2411-2491
 *   狩猎                                              约 2492-2624
 *   地图                                              约 2625-3019
 *   正文渲染与生成 · 展示与文本处理                      约 3020-3484
 *   开场白                                             约 3485-3871
 *   正文渲染与生成 · 发送与生成/删除                     约 3872-4373
 *   变量页(renderVarTab)                               约 4374-4436
 *   渲染总控(renderAll)                                约 4437-4464
 *   事件绑定与生命周期 · 变量事件监听                     约 4465-4507
 *   事件绑定与生命周期 · 切聊天重载                       约 4508-4530
 *   init                                              约 4531-4576
 *   事件绑定与生命周期 · 快捷键与卸载清理                 约 4577-末尾
 *
...
/* 段间距由这里定死, 不再取决于模型输出了一个还是三个换行(见 storyParas)。
   不用首行缩进(2em 缩进观感偏书面, 与这套界面不搭), 所以段距要给足 —— 缩进与段距是两种互斥的
...
`;
  ...
  // 立绘全库: 每角色 7 日常主题(各3张) + 9 张 NSFW(degrade 字段, 3组×3张) = 30 张, 图床外链
  const GAL = {
  ...
  // 舱室设定与配套世界书一致; 不画船壳与房间框, 空间关系全靠图标方位表达.
  ...
  // 海岸线: 经纬度多段线(加拿大北极群岛主要轮廓, 取自 Natural Earth 50m 陆地数据裁剪+简化), 与兴趣点同投影
  ...
  // 报错文案严格遵循 PANELS 里 label+'页' 这个拼接规律(现有面板零例外), 不单独维护一份文案表。
  ...
  // 有意行为: 下面这行的修复优先级(任意存活→CAST[0])只在 charSel 还没选过
  // (null)或者指向了非法角色名时才生效, 只会在会话最开始触发一次。角色死亡不会把她移出 CAST 数组,
  // 所以中途死亡不会让 charSel 自动跳到别的角色——死亡不该打断玩家正在看的角色页,
  // 只是外观上叠加"已故"态。不要改成"每次渲染都重新校验存活状态"。
  ...
  // 手指/鼠标交互: 单指(单键)拖动平移, 双指捏合缩放, 桌面滚轮缩放。
  // .exp-chart 上已有 touch-action:none, 浏览器不会抢走多指手势。
  ...
  // 指令楼用第三人称 {{user}} 指代玩家角色: 宏在 /send 的斜杠解析层被替换成 persona 名, 所以
  ...
  async function runGeneration() {
    ...
    // 显示进度(revealedLen)滞后于已到达的文本, 每帧只追差距的约 1/6(等比逼近, 差距越大追得越快,
    // 收敛用时基本恒定): 后端/代理攒批转发时会一次推来一大段, 渐进追赶把"整段突然冒出来"摊成连续几帧的
    // 增长。已到达的文本永远是上限, 不会显示还没生成的内容; 生成结束后 finally 里 cancelAnimationFrame
    // 丢弃未追完的尾巴, 交给权威渲染(renderStoryLog)一次性补全, 这个"跳"只出现一次。
    // 不要简化成直接显示最新全文: 攒批场景的颗粒感会原样回归。
    let revealedLen = 0;
    const flushStream = () => {
      streamRAF = null;
      const log = doc.getElementById(SEL.storyLog);
      ...
      setStoryStatus('生成中...');
      if (revealedLen > target.length) revealedLen = target.length; // 防御性钳位, 正常流程不会走到
  ...
  // ════ 事件绑定与生命周期 · 切聊天重载 ════
  // 本脚本是常驻的"脚本库"型脚本, 运行在自己独立的 iframe 里(所以 doc = window.parent.document),
  // 不随楼层消息生灭。酒馆切换聊天(含同角色开新聊天)默认*不会*销毁重建这个 iframe——不重跑本文件、
  // 不触发 pagehide; 而 mag_variable_update_ended 等事件是全局事件总线、不区分聊天文件, 新聊天照样在
  // 同一个脚本实例上触发。不处理的后果: lastStat/prevStat/storyHtmlCache(按裸 message_id 缓存)停留在旧聊天
  // 状态, 新旧聊天楼层号一重叠, 旧聊天缓存的正文就会顶替新聊天内容显示, 变量 diff 也会比出没道理的跳变。
  // 处理方式: chat id 变了就 reloadIframe()(本质是本 iframe 的 location.reload()), 让 pagehide 清理和
  // init() 各自完整走一遍; 不手写"清哪些缓存/挪哪些DOM", 也不会叠加只在顶层注册一次的监听器(如 resize)。
  // lastKnownChatId 在脚本启动时就用 SillyTavern.getCurrentChatId() 取基准, 不等第一次 CHAT_CHANGED:
  // 脚本注入晚于酒馆首次派发该事件时, 等事件定基准会把玩家第一次真正切聊天误判成"首次建档"而漏判。
  // 同一聊天内重复触发 CHAT_CHANGED(如重新应用正则)时 chat_id 不变, 不重载, 不打断玩家。
  ...
  // 两者任一缺失(非酒馆助手环境)退回原生 DOMContentLoaded, 行为一致但没有报错通知
