(() => {
  ...
  var SHELL_ID = "exp-shell-root";
  var SHELL_TOKEN = "exp_" + Math.random().toString(36).slice(2) + "_" + Date.now();
  var LS_KEYS = {
    tab: "exp_shell_tab",
  ...
  function dbg(tag, e) {
    try {
      if (window.parent.localStorage.getItem("expDebug")) console.warn("[远征dbg]", tag, e);
    } catch (err) {
    }
  }
  ...
  function tierIdx(v) {
    ...
    return v < 26 ? 0 : v < 51 ? 1 : v < 76 ? 2 : 3;
  }
  function noArt(cap) {
    return `<div class="exp-noart">${ICO.noart}<span>${cap}</span></div>`;
  }
  var CAST = ["富兰克林", "克洛泽", "菲茨", "古德瑟", "瑙雅", "茜拉"];
  var zeroByCast = () => CAST.reduce((o, n) => {
    o[n] = 0;
    return o;
  }, {});
  var emptyByCast = () => CAST.reduce((o, n) => {
    o[n] = "";
    return o;
  }, {});
  var DEFAULT = {
    时间: "",
    地点: "航海日志读取异常，请刷新页面或切换聊天",
    身处: "随队",
    物资: 0,
    健康: 0,
    士气: 0,
  ...
  var BAND_TABLE = { 物资: [[75, "充裕"], [50, "渐紧"], [25, "匮乏"], [0, "枯竭"]], 健康: [[75, "全员健康"], [50, "零星减员"], [25, "病员成片"], [0, "十不存一"]], 士气: [[75, "高昂"], [50, "平稳"], [25, "低落"], [0, "崩溃"]], 狩猎技巧: [[75, "名猎手"], [50, "猎手"], [25, "学徒猎人"], [0, "生手"]] };
  function band(v, t) {
    const B = BAND_TABLE[t];
    for (const [m, n] of B) if (v >= m) return n;
    return B.at(-1)[1];
  }
  function bandNames(t) {
    return BAND_TABLE[t].map((x) => x[1]).slice().reverse();
  }
  function bandIdx(v, t) {
    const B = BAND_TABLE[t];
    for (let i = 0; i < B.length; i++) if (v >= B[i][0]) return B.length - 1 - i;
    return 0;
  }
  ...
  function mcol(v) {
    return v >= 75 ? "linear-gradient(90deg,var(--meter-good-a),var(--meter-good-b))" : v >= 50 ? "linear-gradient(90deg,var(--meter-mid-a),var(--meter-mid-b))" : v >= 25 ? "linear-gradient(90deg,var(--meter-warn-a),var(--meter-warn-b))" : "linear-gradient(90deg,var(--meter-bad-a),var(--meter-bad-b))";
  }
  ...
  function poiOf(loc) {
    ...
    if (!seg) return null;
    return POI.find((p) => p.key === seg || p.key.includes(seg) || seg.includes(p.key) || (p.别名 || []).some((a) => seg.includes(a) || a.includes(seg))) || null;
  }
  function roomOf(loc) {
    ...
    if (!seg) return null;
    return SHIP_ROOMS.find((r) => r.key === seg || r.key.includes(seg) || seg.includes(r.key) || (r.别名 || []).some((a) => seg.includes(a) || a.includes(seg))) || null;
  }
  ...
  // src/05-data.js
  var ICONS = { "狩猎技巧": "<svg viewBox='9 -12 535 535' fill='currentColor'><path d='M331.734 20.443a4.421 4.421 0 0 0-1.802.327c-27.736 11.543-47.295 57.495-29.899 76.671 33.52 38.946 72.835 55.573 90.147 128.434 2.607 20.15 1.218 40.094 0 60.25-17.312 72.861-56.627 89.488-90.147 128.434-17.396 19.176 2.163 65.128 29.899 76.671 9.038 3.762 28.025-26.165 21.752-25.209-16.34 2.491-37.8-20.941-28.387-28.93 38.47-32.65 105.49-100.055 100.277-135.552-2.211-15.057-9.35-30.36-15.574-45.539 6.225-15.18 13.363-30.482 15.574-45.54 5.214-35.496-61.806-102.901-100.277-135.552-9.412-7.988 12.047-31.42 28.387-28.93 5.881.897-10.44-25.35-19.95-25.535zM152 24.23l-21.441 53.602L152 99.273l21.441-21.441zm-9 91.497v296.546l9-9 9 9V115.727l-2.637 2.636-6.363 6.364zm160 9.847v260.824l18-17.53V143.104zM152 428.727l-23 23v38.546l23-23 23 23v-38.546z'/></svg>", "物资": "<svg viewBox='9 11 493 493' fill='currentColor'><path d='M256 41c-43.696 0-83.28 3.58-111.37 9.197-14.047 2.81-25.26 6.196-32.21 9.483-3.476 1.643-5.842 3.293-6.88 4.306l-.013.014.014.014c1.038 1.013 3.404 2.663 6.88 4.306 6.95 3.287 18.163 6.674 32.21 9.483C172.72 83.42 212.303 87 256 87s83.28-3.58 111.37-9.197c14.047-2.81 25.26-6.196 32.21-9.483 3.476-1.643 5.842-3.293 6.88-4.306l.013-.014-.014-.014c-1.038-1.013-3.404-2.663-6.88-4.306-6.95-3.287-18.163-6.674-32.21-9.483C339.28 44.58 299.697 41 256 41zm-80 15a32 8 0 0 1 32 8 32 8 0 0 1-32 8 32 8 0 0 1-32-8 32 8 0 0 1 32-8zm-75.168 26.594c-2.832 12.035-7.414 32.162-12.05 55.28 16.735 4.338 33.52 7.99 50.327 10.995 2.988-17.203 6.707-34.438 11.27-51.708-3.186-.547-6.3-1.113-9.282-1.71-14.91-2.98-27.13-6.49-36.37-10.86-1.363-.644-2.656-1.307-3.896-1.998zm310.336 0c-1.24.69-2.533 1.354-3.895 1.998-9.24 4.37-21.462 7.88-36.37 10.86-2.93.587-5.99 1.142-9.116 1.68 5.27 16.954 9.544 34.033 12.953 51.22 16.26-2.983 32.412-6.568 48.424-10.754-4.617-23-9.175-43.017-11.996-55.004zm-67.4 17.238c-23.065 2.982-49.9 4.803-78.768 5.117v54.198c30.885-.445 61.603-3.05 91.975-7.773-3.45-17.334-7.805-34.523-13.207-51.543zm-175.475.008c-4.647 17.345-8.416 34.67-11.426 51.98 30.062 4.54 60.16 6.967 90.133 7.354V104.95c-28.842-.314-55.656-2.133-78.707-5.11zm-84.38 55.277l-5.518 30.088c128.542 30.936 239.89 29.948 353.384.137l-4.98-30.172c-110.776 28.798-228.035 29.785-342.886-.053zm350.634 48.176c-16.95 4.406-33.876 8.174-50.83 11.312 3.656 47.603 1.776 95.87-3.55 144.49 18.6-3.803 36.796-8.527 54.468-14.17C439.592 314.762 439 291.606 439 256c0-14.915-1.77-33.334-4.453-52.707zm-357.13.256C74.758 222.827 73 241.15 73 256c0 23.794 4.678 57.228 10.424 89.404 16.604 4.828 33.386 8.97 50.27 12.418-4.532-47.516-6.03-95.247-2.577-143.222-17.624-3.063-35.507-6.74-53.7-11.05zm71.546 13.944c-3.336 47.978-1.63 95.883 3.164 143.813 31.553 5.49 63.348 8.592 94.873 9.33V225.94c-31.995-.576-64.57-3.38-98.037-8.446zm216.902.19c-33.303 5.275-66.792 8.068-100.865 8.34V370.8c32.816-.174 65.224-2.93 96.64-8.25 5.61-49.032 7.722-97.417 4.225-144.866zM86.66 364.93l8.29 31.9c104.15 32.39 225.75 32.428 326.077.733l8.272-32.264c-106.024 31.367-228.01 31.34-342.64-.37zm11.236 51.666c3.816 16.945 6.585 28.183 6.704 28.662.792 2.185 4.694 6.427 12.96 10.37 7.587 3.616 18.215 6.947 30.77 9.704-2.132-12.566-4.142-25.147-6.016-37.74-15.03-3.066-29.865-6.733-44.418-10.996zm318.366 1.31c-14.934 4.36-30.254 8.052-45.852 11.086-2.007 12.08-4.16 24.172-6.43 36.272 12.422-2.745 22.935-6.05 30.46-9.637 8.376-3.994 12.302-8.315 13.02-10.473 3.26-9.78 6.178-18.815 8.802-27.248zm-255.217 13.18c1.917 12.574 3.97 25.154 6.144 37.74 23.637 3.684 51.525 5.748 79.81 6.11V439.24c-28.815-.644-57.66-3.36-85.955-8.154zm190.55 1.223c-28.306 4.484-57.373 6.847-86.595 7.07v35.556c28.358-.363 56.317-2.437 79.994-6.14 2.33-12.19 4.538-24.353 6.602-36.487z'/></svg>", "健康": "<svg viewBox='-17 -17 545 545' fill='currentColor'><path d='M196 16a30 30 0 0 0-30 30v120H46a30 30 0 0 0-30 30v120a30 30 0 0 0 30 30h120v120a30 30 0 0 0 30 30h120a30 30 0 0 0 30-30V346h120a30 30 0 0 0 30-30V196a30 30 0 0 0-30-30H346V46a30 30 0 0 0-30-30H196z'/></svg>", "士气": "<svg viewBox='-10 -13 543 543' fill='currentColor'><path d='M356.688 19.188c-6.83-.032-12.837.64-18.125 1.843-24.178 5.495-36.437 21.983-50.938 41.157-14.5 19.175-31.317 40.993-62.78 47.47C195.08 115.78 154.27 108.253 91.25 78.5c-10.013 44.88-33.406 128.62-60.906 178.656 60.093 28.5 97.245 34.926 121 30.875.01 0 .02.004.03 0 21.59-5.827 34.487-20.094 47.876-43.092 17.014-29.227 32.563-72.198 60.25-123.188l16.406 8.938c-16.69 30.735-28.802 58.617-40 82.937 8.552-6.512 18.633-11.77 31.063-14.594 27.71-6.296 65.053-.495 121.655 24.75-6.932-29.276-1.885-61.913 9.875-92.218 12.686-32.69 33.038-62.907 56.28-84.03-42.595-19.553-73.152-27.554-95.124-28.282-1.01-.033-1.993-.058-2.97-.063zm127.54 14.144c-.858-.025-1.752.062-2.664.266-4.378.977-8.94 4.424-12.084 11.097L289.53 497.31h23.61L490.972 49.368c3.475-10.153-.75-15.86-6.746-16.035z'/></svg>" };
  var EMBLEM = "<svg viewBox='0 0 24 24' fill='none' stroke='currentColor' stroke-width='1.5' stroke-linecap='round'><circle cx='12' cy='12' r='6.2'/><circle cx='12' cy='12' r='2.1'/><path d='M12 2.2v7.7M12 14.1v7.7M2.2 12h7.7M14.1 12h7.7M5.07 5.07l5.45 5.45M13.48 13.48l5.45 5.45M18.93 5.07l-5.45 5.45M10.52 13.48l-5.45 5.45'/></svg>";
  var GAL = {
    "富兰克林": { normal: [{ k: "bedroom", label: "卧房", imgs: ["https://i.postimg.cc/C1dH2zfk/franklin-bedroom-1.png", "https://i.postimg.cc/FR1g67J0/franklin-bedroom-2.png", "https://i.postimg.cc/qRgXFzC8/franklin-bedroom-3.png"] }, { k: "cabin", label: "船长室", imgs: ["https://i.postimg.cc/mDhNntF3/franklin-cabin-1.png", "https://i.postimg.cc/263FtVB7/franklin-cabin-2.png", "https://i.postimg.cc/rwcGkhrX/franklin-cabin-3.png"] }, { k: "deck", label: "露天甲板", imgs: ["https://i.postimg.cc/8zDdSy6p/franklin-deck-1.png", "https://i.postimg.cc/YCkNHXWq/franklin-deck-2.png", "https://i.postimg.cc/6pwVXHv6/franklin-deck-3.png"] }, { k: "gala", label: "晚宴", imgs: ["https://i.postimg.cc/v8f6ZryC/franklin-gala-1.png", "https://i.postimg.cc/DfLbwrh9/franklin-gala-2.png", "https://i.postimg.cc/pVznLfxN/franklin-gala-3.png"] }, { k: "mess", label: "军官起居室", imgs: ["https://i.postimg.cc/9XZqQdVX/franklin-mess-1.png", "https://i.postimg.cc/qByCvs0t/franklin-mess-2.png", "https://i.postimg.cc/HsTjWFBM/franklin-mess-3.png"] }, { k: "tea", label: "品茶", imgs: ["https://i.postimg.cc/8PTskxtt/franklin-tea-1.png", "https://i.postimg.cc/3J3Wrzn1/franklin-tea-2.png", "https://i.postimg.cc/m2ThZK8w/franklin-tea-3.png"] }, { k: "unwind", label: "闲憩", imgs: ["https://i.postimg.cc/DwZZLLVc/franklin-unwind-1.png", "https://i.postimg.cc/D06vbyBJ/franklin-unwind-2.png", "https://i.postimg.cc/PJyX8r2Z/franklin-unwind-3.png"] }], degrade: [["https://i.postimg.cc/qvLsM8sC/franklin-degrade1-1.png", "https://i.postimg.cc/VkRqLjqC/franklin-degrade1-2.png", "https://i.postimg.cc/wTYLXWFq/franklin-degrade1-3.png"], ["https://i.postimg.cc/CLpkGvJ5/franklin-degrade2-1.png", "https://i.postimg.cc/L6FLtyvX/franklin-degrade2-2.png", "https://i.postimg.cc/7Yr03BXL/franklin-degrade2-3.png"], ["https://i.postimg.cc/KvhBP9QK/franklin-degrade3-1.png", "https://i.postimg.cc/7Yr03BXG/franklin-degrade3-2.png", "https://i.postimg.cc/8kMrzRGn/franklin-degrade3-3.png"]] },
    "克洛泽": { normal: [{ k: "arctic", label: "北极", imgs: ["https://i.postimg.cc/90R2zQ3b/crozier-arctic-1.png", "https://i.postimg.cc/QC9DVdLf/crozier-arctic-2.png", "https://i.postimg.cc/ZnBm05kf/crozier-arctic-3.png"] }, { k: "engine", label: "轮机舱", imgs: ["https://i.postimg.cc/BQC36yky/crozier-engine-1.png", "https://i.postimg.cc/Kv5bzVwH/crozier-engine-2.png", "https://i.postimg.cc/xTPQC7hZ/crozier-engine-3.png"] }, { k: "gala", label: "晚宴", imgs: ["https://i.postimg.cc/65hK3Sg1/crozier-gala-1.png", "https://i.postimg.cc/Kv5bzVCb/crozier-gala-2.png", "https://i.postimg.cc/gkKpJ512/crozier-gala-3.png"] }, { k: "log", label: "航海志", imgs: ["https://i.postimg.cc/DfrkjHvc/crozier-log-1.png", "https://i.postimg.cc/zDCZt6zp/crozier-log-2.png", "https://i.postimg.cc/8kRQK9pZ/crozier-log-3.png"] }, { k: "melancholy", label: "独酌", imgs: ["https://i.postimg.cc/qBsVbSJm/crozier-melancholy-1.png", "https://i.postimg.cc/HsTGfzn3/crozier-melancholy-2.png", "https://i.postimg.cc/gkG9fsjN/crozier-melancholy-3.png"] }, { k: "sled", label: "雪橇", imgs: ["https://i.postimg.cc/3xWqTcTg/crozier-sled-1.png", "https://i.postimg.cc/fRkPNrN7/crozier-sled-2.png", "https://i.postimg.cc/MpXLSFSm/crozier-sled-3.png"] }, { k: "vigil", label: "守夜", imgs: ["https://i.postimg.cc/t47cjMjt/crozier-vigil-1.png", "https://i.postimg.cc/Bntz0V0w/crozier-vigil-2.png", "https://i.postimg.cc/DwmNTMTN/crozier-vigil-3.png"] }], degrade: [["https://i.postimg.cc/BbL3tnWY/crozier-degrade1-1.png", "https://i.postimg.cc/T1z6bvfX/crozier-degrade1-2.png", "https://i.postimg.cc/kG0mSPq9/crozier-degrade1-3.png"], ["https://i.postimg.cc/44Dytr8z/crozier-degrade2-1.png", "https://i.postimg.cc/y6Mx9wfy/crozier-degrade2-2.png", "https://i.postimg.cc/44Dytr8b/crozier-degrade2-3.png"], ["https://i.postimg.cc/nVyr76dk/crozier-degrade3-1.png", "https://i.postimg.cc/J4jnQPg3/crozier-degrade3-2.png", "https://i.postimg.cc/MK1TYd4d/crozier-degrade3-3.png"]] },
    "菲茨": { normal: [{ k: "battle", label: "战斗", imgs: ["https://i.postimg.cc/QdBfPKQz/fitz-battle-1.png", "https://i.postimg.cc/2Sq0JLQP/fitz-battle-2.png", "https://i.postimg.cc/wjtwPR58/fitz-battle-3.png"] }, { k: "bedroom", label: "卧房", imgs: ["https://i.postimg.cc/jSWM9nHB/fitz-bedroom-1.png", "https://i.postimg.cc/Z51whYjh/fitz-bedroom-2.png", "https://i.postimg.cc/y8wLz6nC/fitz-bedroom-3.png"] }, { k: "bunny", label: "兔女郎", imgs: ["https://i.postimg.cc/0QZGr9G7/fitz-bunny-1.png", "https://i.postimg.cc/gJ4yrGyK/fitz-bunny-2.png", "https://i.postimg.cc/XJxKXnKf/fitz-bunny-3.png"] }, { k: "dinner", label: "晚餐", imgs: ["https://i.postimg.cc/FRKdW3cp/fitz-dinner-1.png", "https://i.postimg.cc/9MfR8ZTJ/fitz-dinner-2.png", "https://i.postimg.cc/G2SBZtJG/fitz-dinner-3.png"] }, { k: "gala", label: "晚宴", imgs: ["https://i.postimg.cc/ZRqB7p3w/fitz-gala-1.png", "https://i.postimg.cc/k4hVkGFs/fitz-gala-2.png", "https://i.postimg.cc/V6HS3vBG/fitz-gala-3.png"] }, { k: "medal", label: "授勋", imgs: ["https://i.postimg.cc/WpCSQ68T/fitz-medal-1.png", "https://i.postimg.cc/jqmvGQcS/fitz-medal-2.png", "https://i.postimg.cc/1R2MLcB4/fitz-medal-3.png"] }, { k: "portrait", label: "肖像", imgs: ["https://i.postimg.cc/tRwDLtkJ/fitz-portrait-1.png", "https://i.postimg.cc/CMXJWH7Z/fitz-portrait-2.png", "https://i.postimg.cc/FR1g67JL/fitz-portrait-3.png"] }], degrade: [["https://i.postimg.cc/63zVTBVd/fitz-degrade1-1.png", "https://i.postimg.cc/Y0s1f8sF/fitz-degrade1-2.png", "https://i.postimg.cc/j2FzQvFJ/fitz-degrade1-3.png"], ["https://i.postimg.cc/DZfs9tVz/fitz-degrade2-1.png", "https://i.postimg.cc/Y92g5Jch/fitz-degrade2-2.png", "https://i.postimg.cc/MTZRChkX/fitz-degrade2-3.png"], ["https://i.postimg.cc/26jvsPpb/fitz-degrade3-1.png", "https://i.postimg.cc/C1vDSmYB/fitz-degrade3-2.png", "https://i.postimg.cc/rmhWMQcS/fitz-degrade3-3.png"]] },
    "古德瑟": { normal: [{ k: "cabin", label: "夜谈", imgs: ["https://i.postimg.cc/X7rqyncr/goodsir-cabin-1.png", "https://i.postimg.cc/kX2Gt7Ft/goodsir-cabin-2.png", "https://i.postimg.cc/15btGbJm/goodsir-cabin-3.png"] }, { k: "deck", label: "速写", imgs: ["https://i.postimg.cc/281yBCFL/goodsir-deck-1.png", "https://i.postimg.cc/Gh62F6qs/goodsir-deck-2.png", "https://i.postimg.cc/sDbXYbwP/goodsir-deck-3.png"] }, { k: "gala", label: "晚宴", imgs: ["https://i.postimg.cc/T10dWYC5/goodsir-gala-1.png", "https://i.postimg.cc/RhdSnVXt/goodsir-gala-2.png", "https://i.postimg.cc/qqwJCMjc/goodsir-gala-3.png"] }, { k: "investigate", label: "验铅", imgs: ["https://i.postimg.cc/K8wZTWrW/goodsir-investigate-1.png", "https://i.postimg.cc/FH89S6gw/goodsir-investigate-2.png", "https://i.postimg.cc/Sx3y9P75/goodsir-investigate-3.png"] }, { k: "medbay", label: "医务室", imgs: ["https://i.postimg.cc/L8wHPWBw/goodsir-medbay-1.png", "https://i.postimg.cc/4xqJc0Qf/goodsir-medbay-2.png", "https://i.postimg.cc/0NS90CcL/goodsir-medbay-3.png"] }, { k: "novel", label: "偷读", imgs: ["https://i.postimg.cc/k5Wdbg5w/goodsir-novel-1.png", "https://i.postimg.cc/xCYVyVkz/goodsir-novel-2.png", "https://i.postimg.cc/qR0HXHtn/goodsir-novel-3.png"] }, { k: "specimen", label: "标本", imgs: ["https://i.postimg.cc/66m1BN9b/goodsir-specimen-1.png", "https://i.postimg.cc/66m1BN9P/goodsir-specimen-2.png", "https://i.postimg.cc/PfRFdktR/goodsir-specimen-3.png"] }], degrade: [["https://i.postimg.cc/vBMwLw1n/goodsir-nsfw1-1.png", "https://i.postimg.cc/V61PBPSR/goodsir-nsfw1-2.png", "https://i.postimg.cc/63wxLx7Y/goodsir-nsfw1-3.png"], ["https://i.postimg.cc/pdfgdc09/goodsir-nsfw2-1.png", "https://i.postimg.cc/5tBZtKPL/goodsir-nsfw2-2.png", "https://i.postimg.cc/rp1vpZh5/goodsir-nsfw2-3.png"], ["https://i.postimg.cc/rp1vpZhG/goodsir-nsfw3-1.png", "https://i.postimg.cc/XJntBKhL/goodsir-nsfw3-2.png", "https://i.postimg.cc/W3sxqmKS/goodsir-nsfw3-3.png"]] },
    "瑙雅": { normal: [{ k: "festival", label: "祭典", imgs: ["https://i.postimg.cc/BbHhhXdw/nauja-festival-1.png", "https://i.postimg.cc/bYtFYBsz/nauja-festival-2.png", "https://i.postimg.cc/KcMscHRm/nauja-festival-3.png"] }, { k: "hunt", label: "狩猎", imgs: ["https://i.postimg.cc/HWyPWhVx/nauja-hunt-1.png", "https://i.postimg.cc/5NWKXSCg/nauja-hunt-2.png", "https://i.postimg.cc/15178rqC/nauja-hunt-3.png"] }, { k: "igloo", label: "冰屋", imgs: ["https://i.postimg.cc/xCrM6tYw/nauja-igloo-1.png", "https://i.postimg.cc/B6xDkHQz/nauja-igloo-2.png", "https://i.postimg.cc/PxmZ9Yrc/nauja-igloo-3.png"] }, { k: "shaman", label: "萨满", imgs: ["https://i.postimg.cc/pTNzsqRR/nauja-shaman-1.png", "https://i.postimg.cc/0QpmFDyg/nauja-shaman-2.png", "https://i.postimg.cc/Qtk90Dfx/nauja-shaman-3.png"] }, { k: "shelter", label: "庇护", imgs: ["https://i.postimg.cc/tTtZ2bBT/nauja-shelter-1.png", "https://i.postimg.cc/rmC0QTfz/nauja-shelter-2.png", "https://i.postimg.cc/Hx4cBm6n/nauja-shelter-3.png"] }, { k: "tent", label: "兽皮帐", imgs: ["https://i.postimg.cc/rmC0QTfr/nauja-tent-1.png", "https://i.postimg.cc/B6cLM3m2/nauja-tent-2.png", "https://i.postimg.cc/8z27Gc3p/nauja-tent-3.png"] }, { k: "tundra", label: "苔原", imgs: ["https://i.postimg.cc/T3zKx1SY/nauja-tundra-1.png", "https://i.postimg.cc/Nj3KtFVG/nauja-tundra-2.png", "https://i.postimg.cc/t4KsbJf4/nauja-tundra-3.png"] }], degrade: [["https://i.postimg.cc/v8NSGdQk/nauja-degrade1-1.png", "https://i.postimg.cc/rFRZmYs2/nauja-degrade1-2.png", "https://i.postimg.cc/vHx2BSTb/nauja-degrade1-3.png"], ["https://i.postimg.cc/85s9bkp7/nauja-degrade2-1.png", "https://i.postimg.cc/xCqhPjfm/nauja-degrade2-2.png", "https://i.postimg.cc/FR18xFrL/nauja-degrade2-3.png"], ["https://i.postimg.cc/tT7wzRqP/nauja-degrade3-1.png", "https://i.postimg.cc/Y9jZx2rs/nauja-degrade3-2.png", "https://i.postimg.cc/hjh604DY/nauja-degrade3-3.png"]] },
    "茜拉": { normal: [{ k: "butcher", label: "分肉", imgs: ["https://i.postimg.cc/Qd2FDCPC/sila-butcher-1.png", "https://i.postimg.cc/rm8Fyr9F/sila-butcher-2.png", "https://i.postimg.cc/pT2XVFBV/sila-butcher-3.png"] }, { k: "council", label: "议事", imgs: ["https://i.postimg.cc/k4nXMby5/sila-council-1.png", "https://i.postimg.cc/d1qtQy91/sila-council-2.png", "https://i.postimg.cc/50xN9CmC/sila-council-3.png"] }, { k: "healer", label: "医者", imgs: ["https://i.postimg.cc/BbPWJSR3/sila-healer-1.png", "https://i.postimg.cc/Fzkthrw1/sila-healer-2.png", "https://i.postimg.cc/j2nVRxBL/sila-healer-3.png"] }, { k: "igloo", label: "冰屋", imgs: ["https://i.postimg.cc/J4w7tvxF/sila-igloo-1.png", "https://i.postimg.cc/VLys5pgx/sila-igloo-2.png", "https://i.postimg.cc/HsGWjR3G/sila-igloo-3.png"] }, { k: "tent", label: "兽皮帐", imgs: ["https://i.postimg.cc/VLys5p4c/sila-tent-1.png", "https://i.postimg.cc/xTKfm5ph/sila-tent-2.png", "https://i.postimg.cc/zXnzh7PY/sila-tent-3.png"] }, { k: "trade", label: "交易", imgs: ["https://i.postimg.cc/3xSP0D7K/sila-trade-1.png", "https://i.postimg.cc/YCnJLmkM/sila-trade-2.png", "https://i.postimg.cc/qvb96CrW/sila-trade-3.png"] }, { k: "tundra", label: "苔原", imgs: ["https://i.postimg.cc/XYHTBy3W/sila-tundra-1.png", "https://i.postimg.cc/sg6tGZyX/sila-tundra-2.png", "https://i.postimg.cc/fyDHrwZp/sila-tundra-3.png"] }], degrade: [["https://i.postimg.cc/ydYsCzVY/sila-degrade1-1.png", "https://i.postimg.cc/RFV4xzMN/sila-degrade1-2.png", "https://i.postimg.cc/635txN90/sila-degrade1-3.png"], ["https://i.postimg.cc/HxZHpPx8/sila-degrade2-1.png", "https://i.postimg.cc/1tCP9Yt6/sila-degrade2-2.png", "https://i.postimg.cc/hjygS3jr/sila-degrade2-3.png"], ["https://i.postimg.cc/wMRK9qnK/sila-degrade3-1.png", "https://i.postimg.cc/sxGF3jbR/sila-degrade3-2.png", "https://i.postimg.cc/2yLpCkgD/sila-degrade3-3.png"]] }
  };
  var GEO = { W: -131, E: -69, N: 77, S: 65 };
  var COSLAT = Math.cos((GEO.N + GEO.S) / 2 * Math.PI / 180);
  var MAPW = 1e3;
  var MAPH = Math.round(MAPW * (GEO.N - GEO.S) / ((GEO.E - GEO.W) * COSLAT));
  var MAPDW = Math.round(MAPW * 1.15);
  var MAPDH = Math.round(MAPH * 1.15);
  var projX = (lon) => (lon - GEO.W) / (GEO.E - GEO.W) * MAPW;
  var projY = (lat) => (GEO.N - lat) / (GEO.N - GEO.S) * MAPH;
  var POI = [
  ...
  var SHIPICO = "<path d='M-8 2h16l-2.6 5H-5.4Z'/><path d='M0 2V-9'/><path d='M0.5 -9l6 5.5H0.5Z'/>";
  var ROOMICO = {
  ...
  var SHIP_ROOMS = [
    ...
    },
    {
      ...
      px: 297,
      py: 450,
      desc: "下层最尾端、紧邻军官起居室的宽敞舱室，船长在这里起居、办公、铺开海图议事，是全船最体面的房间",
    ...
    },
    {
      ...
      px: 419,
      py: 650,
      desc: "深处的储物舱，堆放够吃几年的口粮：成箱的锡焊罐头、腌肉、饼干、面粉和桶装烈酒，由事务长清点看管",
  ...
  var REGIONS = [
    ...
    { key: "威廉王岛", idx: 3, poly: [[-99.5, 70], [-95.5, 70], [-95, 68], [-98.8, 67.6], [-99.5, 68.5]], labelAt: [-97.4, 69] },
  ...
  var REGION_ALIAS = { "兰开斯特水道": 0, "兰开斯特": 0, "兰开斯特海峡": 0, "比奇越冬海域": 1, "比奇": 1, "比奇岛": 1, "维多利亚困冰区": 2, "维多利亚": 2, "维多利亚海峡": 2, "困冰": 2, "威廉王岛": 3, "威廉王": 3, "威廉": 3, "因纽特营地与南岸": 4, "因纽特": 4, "南岸": 4, "南方": 4, "涅齐里克": 4, "西北航道": 5, "西北": 5 };
  function regionIdxOf(loc) {
    const s = ((loc || "").split("／")[0] || "").trim();
    if (s) {
      if (REGION_ALIAS[s] != null) return REGION_ALIAS[s];
      for (const k in REGION_ALIAS) if (s.includes(k)) return REGION_ALIAS[k];
    }
    const p = poiOf(loc);
    if (p && p.区 != null) return p.区;
    return 0;
  }
  ...
  var THEME_CSS = Object.entries(THEME_VARS).map(([k, v]) => themeVarsCss(k, v)).join("\n");
  var SHELL_CSS = `
...
#exp-shell-root .exp-story-text .exp-inner{color:var(--text-dim,var(--text-faint));}
...
/* 顶栏退出 ✕ */
#exp-shell-root .exp-tb-close{flex:none;margin-left:auto;width:34px;height:34px;display:flex;align-items:center;justify-content:center;cursor:pointer;background:transparent;border:none;color:var(--text-dim);transition:.15s;}
#exp-shell-root .exp-tb-close:hover{color:var(--gold-hi);transform:translateY(-2px);}
...
`;
  ...
  function animateTab(name, isBoot) {
    ...
    if (!panel) return;
    panel.querySelectorAll(".exp-in").forEach((el) => {
  ...
  function playStatFx() {
    ...
    Object.entries(d).forEach(([key, ch]) => {
      root.querySelectorAll('[data-stat="' + key + '"]').forEach((box) => {
        ...
        if (ch.text) {
          animateOnce(box, box.classList.contains("exp-tb-item") ? "exp-tb-flash" : "exp-fade-in");
          return;
        }
  ...
  // src/17-var-render.js
  function collectChangedPaths(prev, cur, prefix, out) {
    if (_.isEqual(prev, cur)) return;
    const bothObj = cur !== null && typeof cur === "object" && prev !== null && typeof prev === "object" && Array.isArray(cur) === Array.isArray(prev);
    if (!bothObj) {
      out.add(prefix);
      return;
    }
  ...
  function renderAll(full, precomputedD) {
    ...
    const targets = full || !activeName ? PANELS.filter((p) => p.render) : PANELS.filter((p) => p.render && p.key === activeName);
  ...
  function parseYamlField(body, field) {
    ...
    if (!block) return "";
    return block[1].split("\n").map((l) => (l.match(/^[ \t]*-[ \t]*(.+?)[ \t]*$/) || [])[1]).filter(Boolean).join("、");
  }
    ...
    try {
      if (typeof setChatMessages === "function") {
        ...
        try {
          ...
          console.warn("[航海日志] 补发swipe事件失败", e);
        }
      }
      storyCacheDrop(0);
      let sd = null;
      ...
      setLastStat(sd && sd.stat_data ? _.cloneDeep(_.omit(sd.stat_data, ["$internal"])) : null);
      setPrevStat(null);
    } catch (e) {
  ...
  function customRoleInfo(name) {
    for (const g of CUSTOM_ROLES) {
      const r = g.roles.find((x) => x[0] === name);
      if (r) return { desc: r[1], officer: !!g.officer };
    }
    return null;
  }
  var CUSTOM_STAGES = [
  ...
  var openingView = "cards";
  var CUSTOM_SWIPE = 1;
  var customForm = { ship: "幽冥号", role: null, stage: null, look: "", past: "" };
  ...
  function renderCustomForm(panel) {
    ...
    const shipsHtml = ["幽冥号", "惊恐号"].map((s) => opt("ship", `data-ship="${s}"`, f.ship === s, `<b>${s}</b><span>${s === "幽冥号" ? "富兰克林船长的旗舰" : "克洛泽船长的座舰"}</span>`)).join("");
    const rolesHtml = CUSTOM_ROLES.map((g) => `<div class="exp-custom-grp">${g.group}</div><div class="exp-custom-opts">` + g.roles.map((r) => opt("role", `data-role="${escapeHtml(r[0])}"`, f.role === r[0], `<b>${escapeHtml(r[0])}</b><span>${escapeHtml(r[1])}</span>`)).join("") + "</div>").join("");
    const stagesHtml = '<div class="exp-custom-opts">' + CUSTOM_STAGES.map((s) => opt("stage", `data-stage="${s.key}"`, f.stage === s.key, `<b>${s.name}</b><i>${s.time.split(" ")[0]}／${s.region}</i><span>${s.blurb}</span>`)).join("") + "</div>";
    panel.innerHTML = `
    <div class="exp-open">
    ...
    doc.getElementById(SEL.customBack).addEventListener("click", () => {
      openingView = "cards";
      renderOpeningTab();
    });
    panel.querySelectorAll(".exp-custom-opt.ship").forEach((b) => b.addEventListener("click", () => {
      f.ship = b.dataset.ship;
      renderCustomForm(panel);
    }));
    panel.querySelectorAll(".exp-custom-opt.role").forEach((b) => b.addEventListener("click", () => {
      f.role = b.dataset.role;
      renderCustomForm(panel);
    }));
    panel.querySelectorAll(".exp-custom-opt.stage").forEach((b) => b.addEventListener("click", () => {
      f.stage = b.dataset.stage;
      renderCustomForm(panel);
    }));
    doc.getElementById(SEL.customLook).addEventListener("input", (e) => {
      f.look = e.target.value;
    });
    doc.getElementById(SEL.customPast).addEventListener("input", (e) => {
      f.past = e.target.value;
    });
  ...
  async function startCustomOpening() {
    ...
    if (!stage || !role) {
      if (hint) hint.textContent = "请先选好职位与登船的时节";
      return;
    }
    customBusy = true;
    try {
  ...
  function openingItems() {
    ...
    info.swipes.forEach((t, i) => {
      if (/<initvar>/i.test(String(t || ""))) items.push({ i, text: t });
    });
  ...
  function renderOpeningTab() {
    ...
    if (selectable && openingView === "custom") {
      renderCustomForm(panel);
      return;
    }
    ...
    const customGroup = items.length && (selectable || cur === CUSTOM_SWIPE) ? sect("自定义") + `<div class="exp-open-cards custom-solo">${customCardHtml(selectable, !selectable && cur === CUSTOM_SWIPE)}</div>` : "";
    let cardsHtml;
    ...
    else cardsHtml = customGroup + (mainCards.length ? sect("正传") + `<div class="exp-open-cards">${mainCards.join("")}</div>` : "") + sect("If线") + `<div class="exp-open-cards">${ifCards.join("")}</div>`;
    panel.innerHTML = `
    <div class="exp-open">
      <div class="exp-open-head">
        <span class="exp-open-emblem">${EMBLEM}</span>
      ...
      ${cardsHtml}
      ${items.length ? `<div class="exp-open-foot">${selectable ? "落笔后随航程锁定，此页转为序章回顾" : "航程已开始，开局随之锁定"}</div>` : ""}
    </div>`;
  ...
  function setOpeningView(v) {
    openingView = v;
  }

  // src/adapters/chatu8.js
  var ILL_TOKEN = "";
  function tokenizeIllustMarkers(s) {
    const re = illustMarkerRe();
    return s.replace(/<image(?:\s[^>]*)?>([\s\S]*?)<\/image\s*>/gi, (m, inner) => {
      const n = (inner.match(re) || []).length;
      return n ? "\n" + Array(n).fill(ILL_TOKEN).join("\n") + "\n" : "";
    }).replace(re, "\n" + ILL_TOKEN + "\n");
  }
  var illustCache = /* @__PURE__ */ new Map();
  ...
  function scanNativeImages(mesId) {
    try {
      ...
      return Array.from(units).map((u) => {
        ...
        return src ? { src, video: media.tagName === "VIDEO" } : null;
      });
    } catch (e) {
      return [];
    }
  ...
  function illustTailHtml(mid, from) {
    ...
    return srcs.slice(from).filter(Boolean).map(
      (item) => '<div class="exp-illust" data-illust-mid="' + mid + '">' + illustMediaHtml(item) + "</div>"
    ).join("");
  }
  function fillIllustSlots(textHtml, mid) {
    ...
    return { html, tail: illustTailHtml(mid, k) };
  }
  ...
  function openIllustLightbox(imgEl) {
    ...
    lb.innerHTML = '<div class="exp-lb-backdrop"></div><div class="exp-lb-stage"><img src="' + escapeHtml(src) + '"></div>';
  ...
  var illustObserver = null;
  function chatu8Node(n) {
    if (!n || n.nodeType !== 1) return false;
    return n.matches && n.matches(".st-chatu8-image-container, .st-chatu8-image-span") || n.querySelector && !!n.querySelector(".st-chatu8-image-container, .st-chatu8-image-span") || n.closest && !!n.closest(".st-chatu8-image-container, .st-chatu8-image-span");
  }
  function initIllustObserver() {
    const chat = doc.getElementById("chat");
    if (!chat || illustObserver) return;
    let raf = 0;
    const pendingMids = /* @__PURE__ */ new Set();
      ...
      for (const m of muts) {
        ...
        if (m.type === "attributes") {
          ...
          if (!hit) for (const n of m.removedNodes) {
            if (chatu8Node(n) || m.target && chatu8Node(m.target)) {
              hit = true;
              break;
            }
          }
  ...
  function pollAcu() {
    updateAcuNav();
    if (!acuPresent() && --acuPollLeft > 0) setTimeout(pollAcu, 2e3);
  }
  var acuHiTimer = null;
  ...
  function openAcuUI() {
    try {
      ...
      console.warn("[航海日志] 打开数据库插件界面失败", e);
    }
  }
  function acuUiOpen() {
    const v2 = doc.getElementById("acu-app-v2");
  ...
  function setGenerating(on) {
    ...
    if (send) {
      send.innerHTML = on ? ICO.stop : ICO.send;
      send.title = on ? "停止" : "发送";
    }
  ...
  async function onDelConfirm() {
    ...
    if (!delArmed) {
      ...
      if (btn) {
        ...
        btn.textContent = "再点一次删除 " + delSel.size + " 楼";
      }
  ...
  function applyUserEdit(log) {
    ...
    turn.innerHTML = '<div class="exp-story-edit"><textarea title="Ctrl+Enter 保存, Esc 取消"></textarea><div class="exp-story-edit-row"><button class="exp-edit-btn exp-edit-cancel">取消</button><button class="exp-edit-btn primary exp-edit-save">保存</button></div></div>';
  ...
  function onStoryTapUp(e) {
    ...
    const turn = e.target.closest(".exp-story-turn.user");
    if (!turn || turn.classList.contains("editing")) {
      lastTap = null;
      return;
    }
    const mid = +turn.dataset.mid;
  ...
  async function runGeneration() {
    ...
    const flushStream = () => {
      ...
      if (!target) {
        ...
        const html = wantThinking ? '<div class="exp-story-thinking"><span class="exp-story-thinking-rule l"></span><span class="exp-story-thinking-ico">' + ICO.thought + '</span><span class="exp-story-thinking-rule r"></span></div>' : '<div class="exp-story-text">' + storyTextHtml("…", true) + "</div>";
  ...
  async function sendText(text, opts) {
    ...
    try {
      ...
      const log = doc.getElementById(SEL.storyLog);
      if (log) {
        const turns = log.querySelectorAll(".exp-story-turn.user");
        animateOnce(turns[turns.length - 1], "exp-in-bubble");
      }
    } catch (e) {
      pendingStop = false;
      setStoryStatus("出错: " + (e && e.message ? e.message : e));
  ...
  function floorData(mid) {
    ...
    try {
      ...
      if (cur) {
        ...
        if (delta && curD.身处 === "营地") ["物资", "健康", "士气"].forEach((k) => {
          delete delta[k];
        });
  ...
  function onStoryLogClick(e) {
    ...
    if (delMode) {
      const head2 = e.target.closest(".exp-story-thought-head");
      if (head2) {
        e.stopPropagation();
        const fold2 = head2.closest(".exp-story-thought");
        const open2 = !fold2.classList.contains("open");
        fold2.classList.toggle("open", open2);
        const midAttr2 = fold2.dataset.foldMid;
        if (midAttr2 != null) {
          const mid = +midAttr2;
          if (open2) thoughtFoldOpen.add(mid);
      ...
      if (turn) {
        ...
        if (mid >= 1) {
          if (delSel.has(mid)) {
            delSel.delete(mid);
            turn.classList.remove("delsel");
          } else {
            delSel.add(mid);
            turn.classList.add("delsel");
          }
          updateDelBar();
        }
      }
    ...
    if (charBtn) {
      e.stopPropagation();
      openChar(charBtn.dataset.footChar);
      return;
    }
  ...
  function ffLine(label, ch, stage) {
    return label + "　" + ch.from + (ch.to > ch.from ? " 升至 " : " 降至 ") + '<span class="new">' + ch.to + '</span>　<span class="stage">' + stage + "</span>";
  }
  ...
  function voiceCardHtml(mid, name, tab) {
    ...
    const body = tab === "memoir" ? list.length ? '<div class="exp-vc-memos">' + list.slice().reverse().map((x) => {
      const m = fmtMemoir(x);
      return '<div class="exp-vc-memo">' + (m.head ? "<b>" + escapeHtml(m.head) + "</b>　" : "") + escapeHtml(m.body) + "</div>";
    }).join("") + "</div>" : '<div class="exp-vc-empty">还没有留下回想</div>' : voice ? '<div class="exp-vc-text">' + escapeHtml(voice) + "</div>" : '<div class="exp-vc-empty">她的心声还没有传到这里</div>';
    const tabBtn = (k, label) => '<button class="exp-vc-tab' + (tab === k ? " on" : "") + '" data-foot-tab="voice:' + name + ":" + k + '">' + label + "</button>";
    return '<div class="exp-vc-wrap"><div class="exp-vc"><button class="exp-vc-img" data-foot-char="' + name + '" title="翻到' + name + '">' + (img ? '<img src="' + escapeHtml(img) + '" alt="' + name + '" onerror="this.style.opacity=.25">' : noArt("立绘待补")) + '</button><div class="exp-vc-main"><div class="exp-vc-head">' + name + '<span class="exp-vc-tabs">' + tabBtn("voice", "心声") + '<span class="exp-vc-sep">｜</span>' + tabBtn("memoir", "回想") + "</span></div>" + body + "</div></div></div>";
  }
  function floorFootInner(mid) {
    ...
    const row = '<div class="exp-ff">' + items.map((it) => '<button class="exp-ff-item' + (openKey === it.key ? " open" : "") + '" data-foot-item="' + it.key + '">' + it.label + ffArrow(it.dv) + "</button>").join("") + '<span class="exp-ff-gap"></span>' + (voices.length ? '<span class="exp-ff-voices"><span class="exp-ff-label">心声</span>' + voices.map((n) => {
      const img = frontImg(n);
      return '<button class="exp-ff-voice' + (openName === n ? " open" : "") + '" data-foot-item="voice:' + n + '">' + (img ? '<img src="' + escapeHtml(img) + '" alt="" onerror="this.style.opacity=.2">' : "") + "<span>" + n + '</span><span class="exp-ff-caret">' + ICO.chev + "</span></button>";
    }).join("") + "</span>" : "") + "</div>";
    let panel = "";
    if (openKey) {
  ...
  function storyTurnHtml(role, text, mid, thought, open, foot) {
    ...
    return '<div class="exp-story-turn ' + cls + '"' + midAttr + titleAttr + ">" + foldHtml + '<div class="exp-story-text">' + textHtml + "</div>" + tail + (foot || "") + "</div>";
  }
  ...
  function optionsHtml(opts) {
    return '<div class="exp-story-options"><div class="exp-story-opthead">行动</div>' + opts.map(
      (t, i) => '<button class="exp-story-opt" data-idx="' + i + '"><span class="exp-story-opt-num">' + (i + 1) + '</span><span class="exp-story-opt-text">' + escapeHtml(t) + "</span></button>"
    ).join("") + "</div>";
  }
  ...
  function storyParas(text, streaming) {
    ...
    const marked = streaming ? escaped : escaped.replace(/“([^”\n]*?)”/g, '<span class="exp-quote">“$1”</span>').replace(/&quot;([^\n]*?)&quot;/g, '<span class="exp-quote">&quot;$1&quot;</span>').replace(/\*\*/g, "").replace(/\*([^*\n]+?)\*/g, '<span class="exp-inner">$1</span>');
    return marked.split(/\n+/).map((s) => s.trim()).filter(Boolean);
  }
  function storyTextHtml(text, streaming) {
    return storyParas(text, streaming).map(
      (s) => s === ILL_TOKEN ? '<p class="exp-illust exp-ill-slot"></p>' : "<p>" + s + "</p>"
    ).join("");
  }
  ...
  function thoughtFoldHtml(thought, mid, open) {
    ...
    return '<div class="exp-story-thought' + (open ? " open" : "") + '"' + midAttr + '><div class="exp-story-thought-head" title="思维链"><span class="exp-story-thought-rule l"></span><span class="exp-story-thought-ico">' + ICO.thought + '</span><span class="exp-story-thought-rule r"></span></div><div class="exp-story-thought-body">' + escapeHtml(thought) + "</div></div>";
  }
  ...
  function castTabsState(D) {
    ...
    if (!charSel || !CAST.includes(charSel)) setCharSel(CAST.find((n) => !isDead(n)) || CAST[0]);
    const html = CAST.map(
      (n) => `<button class="exp-char-tab ${n === charSel ? "active" : ""} ${isDead(n) ? "dead" : ""}" data-name="${n}">${n}${isDead(n) ? '<span class="tab-dead">已故</span>' : ""}</button>`
    ).join("");
    return { isDead, html };
  }
  ...
  function lbItems(name) {
    const items = galItems(name);
    return sfwMode === "sfw" ? items.filter((x) => x.pos !== "back") : items;
  }
  ...
  function galSectionsHTML(name) {
    ...
    const normalRows = g.normal.length ? g.normal.map((t) => `<div class="exp-gal-theme"><span class="exp-gal-lab">${t.label}</span><div class="exp-gal-imgs">${t.imgs.map((u) => galThumb(u, p.front === u, "front", null)).join("")}</div></div>`).join("") : noArt("立绘待补");
    const nsfwRows = g.degrade.length ? g.degrade.map((imgs, ti) => `<div class="exp-gal-theme"><div class="exp-gal-imgs">${imgs.map((u) => galThumb(u, false, "back", ti)).join("")}</div></div>`).join("") : noArt("立绘待补");
  ...
  function bindGalleryPanel(panel, name, D) {
    if (!pins[name]) pins[name] = { front: null };
    const keepScroll = (fn) => {
      const body = panel.querySelector(".exp-gal-body");
      const y = body ? body.scrollTop : 0;
      fn();
      const b2 = panel.querySelector(".exp-gal-body");
    ...
    panel.querySelectorAll(".exp-gal-thumb").forEach((b) => {
      ...
      b.addEventListener("click", () => {
        const items = lbItems(name);
        const idx = items.findIndex((x) => x.url === it.url);
        openLightbox(name, idx < 0 ? 0 : idx);
      });
  ...
  var lbState = null;
  function lbPinned(name, it) {
    const p = pins[name] || {};
    return it.pos === "front" && p.front === it.url;
  }
  function refreshGalleryKeepScroll() {
    ...
    const y = body ? body.scrollTop : 0;
    try {
      renderGalleryTab(readMVU());
    } catch (e) {
      dbg("refreshGallery", e);
    }
  ...
  function renderLightbox(wrap) {
    ...
    const pinBtn = it.pos === "front" ? `<button class="exp-lb-pin${pinned ? " on" : ""}">${ICO.check}<span>${pinned ? "已固定，点击取消" : "固定为显示立绘"}</span></button>` : "";
    wrap.innerHTML = `
    ...
    [lbState.idx - 1, lbState.idx + 1].forEach((i) => {
  ...
  var ROSTER_GROUPS = ["军官", "水手与工匠", "学徒帮工", "因纽特人", "其他"];
  var ROSTER_BADGE = { 患病: "ill", 重伤: "ill", 疯癫: "mad", 死亡: "gone" };
  ...
  function statusTone(st) {
    return isDeadTag(st) ? "gone" : ROSTER_BADGE[st] || "";
  }
  ...
  function renderCrewTab(D) {
    ...
    let fi = 0;
    const rosterBody = alive.length || dead.length ? rosterStatHtml(alive, dead) + groups.map((g) => `<div class="exp-roster-sect"><span>${g.label}</span></div><div class="exp-roster-grid">${g.items.map((it) => rosterCard(it, fi++)).join("")}</div>`).join("") + (dead.length ? `<div class="exp-roster-sect"><span>死者名录</span></div><div class="exp-roster-grid">${dead.map((it) => rosterCard(it)).join("")}</div>` : "") : '<div class="exp-crew-empty">还没跟船上的人混熟</div>';
  ...
  var PREY_TIER = { 25: "学徒猎人", 50: "猎手", 75: "名猎手" };
  var MATES = ["独自", "瑙雅", "茜拉", "部落猎手", "全部落"];
  var MATE_LABEL = { 部落猎手: "几名猎手" };
  var huntSel = { prey: "观学", mates: ["独自"], dogs: false };
  function huntUnlocked(D) {
    return D.身处 === "营地" || D.狩猎技巧 > 0;
  }
  ...
  function huntText(D) {
    ...
    if (huntSel.prey === "观学") {
      let t2 = `跟着${tribe ? "全部落的猎队" : list || "猎队"}出猎，打下手，多看多学`;
      if (huntSel.dogs) t2 += "，带上猎犬队";
      return t2 + "。";
    }
  ...
  function renderHuntTab(D) {
    ...
    panel.querySelectorAll(".exp-prey-card:not(.locked)").forEach((b) => b.addEventListener("click", () => {
      huntSel.prey = b.dataset.prey;
      renderHuntTab(D);
  ...
  var mapFillMode = (panel) => mapCssFlag(panel, "--map-fill");
  var mapPortraitMode = (panel) => mapCssFlag(panel, "--map-portrait");
  function syncViewAspect(panel) {
    ...
    if (Math.abs(a - viewAspect) > 2e-3) viewAspect = a;
  }
  function fitViewToRegions(curIdx) {
    ...
    return clampView({ x: (minX + maxX) / 2 - w / 2, y: (minY + maxY) / 2 - w * viewAspect / 2, w, h: w * viewAspect });
  }
  ...
  function zoomAround(svg, factor, ux, uy) {
    ...
    mapView = clampView({ x: ux - fx * w, y: uy - fy * (w * viewAspect), w, h: w * viewAspect });
    applyView(svg);
  }
  function bindMapInteraction(svg) {
    ...
    function pinchMove() {
      ...
      mapView = clampView({ x: pinch.ux - fx * w, y: pinch.uy - fy * h, w, h });
      applyView(svg);
    }
    ...
    const lift = (e) => {
      ...
      if (pts.size === 1) {
        ...
        drag = { x: p.x, y: p.y, v: Object.assign({}, mapView) };
  ...
  var graticuleCache = null;
  function buildGraticule() {
    if (graticuleCache != null) return graticuleCache;
    let s = "";
    for (let lon = Math.ceil(GEO.W / 10) * 10; lon <= GEO.E; lon += 10) {
      const x = projX(lon).toFixed(1);
      s += `<line class="exp-grid" x1="${x}" y1="0" x2="${x}" y2="${MAPH}"/>`;
    }
    for (let lat = Math.ceil(GEO.S / 2) * 2; lat <= GEO.N; lat += 2) {
      const y = projY(lat).toFixed(1);
      s += `<line class="exp-grid" x1="0" y1="${y}" x2="${MAPW}" y2="${y}"/>`;
    }
  ...
  function showPoiPopup(mapEl, gEl, p, seen) {
    ...
    if (topAbove - ph - 13 < 4) {
      ...
      pop.style.top = gr.bottom - mr.top - gr.height / 2 + "px";
    } else pop.style.top = topAbove + "px";
    pop.style.left = left + "px";
    setPopArrow(pop, anchorX, left, pw);
    pop.querySelector(".exp-poipop-x").addEventListener("click", (e) => {
      e.stopPropagation();
      pop.remove();
    });
  }
  ...
  function renderChartPanel(panel, D, swHtml) {
    ...
    if (!mapView || mapViewAspect !== viewAspect) {
      mapView = fitViewToRegions(curIdx);
      mapViewAspect = viewAspect;
  ...
  function renderShipPanel(panel, D, swHtml) {
    ...
    const DECKS = pv ? [{ cy: 110, name: "露天" }, { cy: 380, name: "下层" }, { cy: 650, name: "深处" }, { cy: 850, name: "底层" }] : [{ cy: 58, name: "露天" }, { cy: 198, name: "下层" }, { cy: 339, name: "深处" }, { cy: 479, name: "底层" }];
    ...
    mapEl.addEventListener("pointerdown", (e) => {
      if (!e.target.closest(".exp-spoi")) removePoiPopup(mapEl);
    });
  ...
  function ensureShell() {
    ...
    root.querySelectorAll(".exp-nav-item[data-tab]").forEach((btn) => {
      btn.addEventListener("click", () => {
        if (btn.dataset.tab === "opening" && openingView !== "cards") {
          setOpeningView("cards");
          renderOpeningTab();
        }
  ...
  function switchTab(name) {
    ...
    if (name === "story") {
      const log = doc.getElementById(SEL.storyLog);
      if (log) {
        log.scrollTop = storyScroll == null ? log.scrollHeight : storyScroll;
        updateJumpBtn();
      }
    }
  ...
  var theme = "dark";
  {
    const t = safeLSGet(LS_KEYS.theme);
    if (t && THEMES.some((x) => x.key === t)) theme = t;
  }
  var OPTION_MODES = [
  ...
  var optionMode = "send";
  {
    const m = safeLSGet(LS_KEYS.optionMode);
    if (OPTION_MODES.some((x) => x.key === m)) optionMode = m;
  }
  ...
  var heroMode = "stable";
  {
    const m = safeLSGet(LS_KEYS.heroMode);
    if (HERO_MODES.some((x) => x.key === m)) heroMode = m;
  }
  ...
  var fontMode = "std";
  {
    const m = safeLSGet(LS_KEYS.fontSize);
    if (FONT_MODES.some((x) => x.key === m)) fontMode = m;
  }
  function applyFontMode(key) {
    ...
    const root = doc.getElementById(SHELL_ID);
    if (root) {
      if (key === "std") root.removeAttribute("data-fontsize");
      else root.setAttribute("data-fontsize", key);
    }
    renderSettingsTab();
  }
  var MOTION_MODES = [
  ...
  var motionMode = "full";
  {
    const m = safeLSGet(LS_KEYS.motion);
    if (MOTION_MODES.some((x) => x.key === m)) motionMode = m;
  }
  function syncMotionAttr() {
    [SHELL_ID, "exp-entry"].forEach((id) => {
      const el = doc.getElementById(id);
      if (!el) return;
      if (motionMode === "lite") el.setAttribute("data-motion", "off");
  ...
  function motionOK() {
    ...
    try {
      if (window.parent.matchMedia("(prefers-reduced-motion:reduce)").matches) return false;
    } catch (e) {
      dbg("motionQuery", e);
    }
    return true;
  ...
  var sfwMode = "nsfw";
  {
    const m = safeLSGet(LS_KEYS.sfw);
    if (SFW_MODES.some((x) => x.key === m)) sfwMode = m;
  }
  function applySfwMode(key) {
    ...
    const root = doc.getElementById(SHELL_ID);
    if (root) {
      if (key === "sfw") root.setAttribute("data-sfw", "");
      else root.removeAttribute("data-sfw");
    }
    renderSettingsTab();
  }
  function applyTheme(key) {
    ...
    const root = doc.getElementById(SHELL_ID);
    if (root) {
      if (key === "dark") root.removeAttribute("data-theme");
      else root.setAttribute("data-theme", key);
    }
    const entry = doc.getElementById(SEL.entry);
    if (entry) {
      if (key === "dark") entry.removeAttribute("data-theme");
      else entry.setAttribute("data-theme", key);
    }
  ...
  // src/06-state-mvu.js
  var isDeadTag = (v) => v === "死亡" || v === "死";
  var charSel = null;
  function loadPins() {
    let p = {};
    try {
      const raw = window.parent.localStorage.getItem(LS_KEYS.pins);
      const parsed = raw ? JSON.parse(raw) : null;
      if (parsed && typeof parsed === "object" && !Array.isArray(parsed)) p = parsed;
    } catch (e) {
    }
    CAST.forEach((n) => {
      p[n] = { front: p[n] && typeof p[n] === "object" && p[n].front || null };
    });
    return p;
  }
  function savePins() {
    safeLSSet(LS_KEYS.pins, JSON.stringify(pins));
  }
  var pins = loadPins();
  var snap = {};
  function snapOf(name) {
    return snap[name] || (snap[name] = { front: null });
  }
  function normalPool(name) {
    const g = GAL[name];
    return g ? g.normal.reduce((a, t) => a.concat(t.imgs), []) : [];
  }
  function pick(arr) {
  ...
  function pickSeeded(arr, seedStr) {
    return arr.length ? arr[Math.floor(seededRand(seedStr) * arr.length)] : "";
  }
  function frontImg(name) {
    const p = pins[name] && pins[name].front;
    if (p) return p;
    const s = snapOf(name);
    if (!s.front) s.front = pick(normalPool(name));
    return s.front;
  }
  var heroRollId = null;
  function maybeRerollHero() {
    ...
    CAST.forEach((n) => {
      const s = snapOf(n);
      s.front = pickSeeded(normalPool(n), lid + ":" + n + ":front");
    });
  }
  var lastStat = null;
  var prevStat = null;
    ...
    try {
      ...
      if (sd) {
        const g = (p, d) => {
          ...
          return x === void 0 || x === null || x === "" ? d : x;
        };
        const cl = (v) => Math.max(0, Math.min(100, v));
        const mapByCast = (prefix, fallback) => CAST.reduce((o, n) => {
          o[n] = g(prefix + "." + n, fallback);
          return o;
        }, {});
        const R = {
          时间: g("时间", ""),
  ...
  async function init() {
    ...
    pollAcu();
    try {
      window.parent.addEventListener("exp-shell-enter", onShellEnter);
    } catch (e) {
  ...
  var PRESSABLE_SEL = ".exp-nav-item,.exp-iconbtn,.exp-story-opt,.exp-story-earlier,.exp-mate-btn,.exp-prey-card,.exp-theme-opt,.exp-hunt-go,.exp-del-btn,.exp-edit-btn,.exp-tb-close,.exp-mapctl button,.exp-entry-pill,.exp-story-thought-head,.exp-story-jump";
  var pressState = null;
